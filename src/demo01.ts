...
class RenderSystem implements System {
  ...
  update(entity: Entity, deltaTime: number): void {
    const position = componentManager.getComponent<Position>(
      entity.id,
      "Position"
    );
    const opacity = componentManager.getComponent<Opacity>(
      entity.id,
      "Opacity"
    );
...
const sceneEntity = entityManager.createEntity();

// Create background and main layers
const backgroundLayer = entityManager.createEntity();

const mainLayer = entityManager.createEntity();

entityManager.setChildren(sceneEntity.id, [backgroundLayer.id, mainLayer.id]);

// Create the player entity
const playerEntity = entityManager.createEntity();

componentManager.addComponent(playerEntity.id, "Position", { x: 100, y: 100 });
componentManager.addComponent(playerEntity.id, "Opacity", { value: 1 });

playerEntity.componentMask |= POSITION | OPACITY;

entityManager.setChildren(mainLayer.id, [playerEntity.id]);

// Create the systems
const renderSystem = new EnhancedRenderSystem();
const fadeSystem = new EnhancedFadeSystem(renderSystem);
const engine = new ECSEngine(entityManager, componentManager);
