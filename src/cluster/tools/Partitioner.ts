...
export class SparseGrid<ID> implements SpatialIndex<ID> {
    ...
    private readonly grid = new Map<number, Map<number, Set<ID>>>();
    // tracks which cells an ID occupies: array of [ix, iy]
    private readonly idToCells = new Map<ID, [number, number][]>();
...
export class Quadtree<ID> implements SpatialIndex<ID> {
    ...
    clear(): void {
        this.items = [];
        this.divided = false;
        // If already subdivided, recursively clear children and remove references
        if (this.nw) this.nw.clear();
        if (this.ne) this.ne.clear();
        if (this.sw) this.sw.clear();
        if (this.se) this.se.clear();
        // Remove child references to allow garbage collection
        this.nw = undefined as any;
        this.ne = undefined as any;
        this.sw = undefined as any;
        this.se = undefined as any;
    }
...
export class LooseQuadtree<ID> implements SpatialIndex<ID> {
    ...
    private readonly looseBoundary: AABB;
    ...
    constructor(
        ...
        private readonly maxDepth: number = 10,
        private readonly depth: number = 0,
        private readonly looseness: number = 1.5
    ) {
...
export class BVH<ID> implements SpatialIndex<ID> {
    ...
    private buildNode(list: Array<{ id: ID; box: AABB }>): BVHNode<ID> | null {
        ...
        list.sort((a, b) => {
            const ca = axis === "x" ? (a.box.minX + a.box.maxX) / 2 : (a.box.minY + a.box.maxY) / 2;
            const cb = axis === "x" ? (b.box.minX + b.box.maxX) / 2 : (b.box.minY + b.box.maxY) / 2;
            return ca - cb;
        });
