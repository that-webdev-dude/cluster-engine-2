 ...
 */
export type ArchetypeV2<S extends readonly ComponentDescriptor[]> = {
...
function register<S extends readonly ComponentDescriptor[]>(...desc: S): S {
    ...
    for (const d of desc) {
        if (seen.has(d.name)) {
            throw new Error(
                `ArchetypeV2.register: Duplicate descriptor name "${d.name}". Use unique names.`
            );
        }
        seen.add(d.name);
    }
...
function format<S extends readonly ComponentDescriptor[]>(
    ...
    return `ArchetypeV2.${arch.name}<${arch.schema
        .map((d) => d.name)
        .join(", ")}> stride = ${arch.byteStride} bytes`;
}

export const ArchetypeV2 = {
    create,
    register,
    matches,
    includes,
