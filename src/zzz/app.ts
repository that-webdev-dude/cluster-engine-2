...
const glRenderSystem = {
    ...
    render: (alpha: number) => {
        ...
        for (let i = 0; i < count * 2; ++i) {
            glRenderSystem.interpPos[i] = prev[i] + (cur[i] - prev[i]) * alpha;
        }

        // rect data from subarrays
        const rectData: RectData = {
            // positions: chunk.views.Position.subarray(0, count * 2),
            positions: glRenderSystem.interpPos.subarray(0, count * 2),
...
export default () => {
    ...
    engine.addRenderable(glRenderSystem);
    engine.start();
};
